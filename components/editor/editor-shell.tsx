...
export default function EditorShell({
  ...
  >(
    diagramId
      ? accountsReady
        ? { status: "loading" }
        : { status: "error", message: "Saved diagrams are not configured here." }
      : null,
  );

  useEffect(() => {
    if (!diagramId || !accountsReady) {
      return;
    }
    let alive = true;
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from("diagrams")
        .select("id, title, category, document")
        .eq("id", diagramId)
        .eq("user_id", user.id)
        .maybeSingle();
      if (!alive) return;
      if (error || !data) {
        setLoaded({
          status: "error",
          message: error ? "This diagram could not be loaded." : "Diagram not found.",
        });
        return;
      }
