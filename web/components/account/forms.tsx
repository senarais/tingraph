...
function actionError(cause: unknown, values?: Record<string, string>): FormState {
  return {
    error: cause instanceof APIError ? cause.message : "The request could not be completed. Try again.",
    values,
  };
}
...
async function requestPasswordReset(_: FormState, form: FormData): Promise<FormState> {
  const email = text(form, "email");
  try {
    return await publicJSON<FormState>("/api/v1/auth/password/forgot", { email });
  } catch (cause) {
    return actionError(cause, { email });
  }
}

async function updateProfile(_: FormState, form: FormData): Promise<FormState> {
  const values = Object.fromEntries(
    Object.keys(PROFILE_FIELDS).map((field) => [field, text(form, field)]),
  );
  try {
...
async function uploadAvatar(_: FormState, form: FormData): Promise<FormState> {
  try {
    const response = await apiFetch("/api/v1/me/avatar", { method: "POST", body: form });
    if (!response.ok) throw await APIError.from(response);
...
export function PasswordForm({ token = "" }: { token?: string }) {
  ...
  const resetToken = token || fragment.get("token") || "";
  const [state, action, pending] = useActionState(
    async (_: FormState, form: FormData): Promise<FormState> => {
      const password = secret(form, "password");
      const confirm = secret(form, "confirm");
      try {
        const result = await publicJSON<FormState>("/api/v1/auth/password/reset", {
          token: resetToken,
          password,
          confirm,
        });
  ...
  if (!resetToken) {
    return <p className="text-[13px] leading-relaxed text-alert">This reset link is missing or invalid. Ask for another one.</p>;
  }
  return (
    <form action={action} className="space-y-4">
      <Input
