...
import { refuses } from "@/lib/ai/chat";
 ...
 * checked against the editor's own parser. A refused first draft is handed
 * back to the Go API once with the parser's exact complaint.
 *
...
export default function AiDrawer({
  ...
  const send = async () => {
    ...
    const request = (retry?: { source: string; complaint: string }) => {
      const form = new FormData();
      form.set("ask", JSON.stringify({
        category,
    ...
    setThinking(true);
    try {
      let response = await apiFetch("/api/v1/ai", {
        method: "POST",
        body: request(),
      });
      if (!response.ok) throw await APIError.from(response);
      let reply = (await response.json()) as { message?: string; source?: string };
      let complaint = reply.source ? refuses(reply.source, store.direction) : null;
      if (reply.source && complaint) {
        response = await apiFetch("/api/v1/ai", {
          method: "POST",
          body: request({ source: reply.source, complaint }),
        });
        if (!response.ok) throw await APIError.from(response);
        const second = (await response.json()) as { message?: string; source?: string };
        complaint = second.source ? refuses(second.source, store.direction) : "nothing came back";
        if (!complaint) reply = second;
      }
      say({
        role: "ai",
        text: complaint
          ? `${reply.message || "I could not write that."}\n\nThe source still would not draw — ${complaint}. Try describing it a little differently.`
          : reply.message || "Something came back empty. Try asking again.",
        source: complaint ? undefined : reply.source || undefined,
      });
    } catch (cause) {
      say({
        role: "ai",
        text:
          cause instanceof APIError
            ? cause.message
            : "I could not reach the server. Check that it is running and try again.",
      });
    } finally {
